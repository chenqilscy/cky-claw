...
class LocalSandbox(SandboxExecutor):
    ...
    async def execute(self, code: str, *, language: str = "python") -> SandboxResult:
        ...
        try:
            if work_dir is None:
                tmp_dir = tempfile.mkdtemp(prefix="ckyclaw_sandbox_")
                work_dir = tmp_dir
            ...
            # 安全限制：移除敏感变量
            for key in ("AWS_SECRET_ACCESS_KEY", "DATABASE_URL", "SECRET_KEY", "CKYCLAW_SECRET_KEY"):
                env.pop(key, None)
            ...
            return SandboxResult(
                exit_code=proc.returncode if not timed_out else -1,  # type: ignore[union-attr]
                stdout=stdout_str,
                stderr=stderr_str,
                timed_out=timed_out,
