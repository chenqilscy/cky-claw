...
## 5.3 CkyClaw Framework vs LangChain/LangGraph 深度对比

> 本节为重构可行性分析的基础材料。如果未来考虑基于 LangChain/LangGraph 重构 CkyClaw Framework，需调整哪些内容。

### 5.3.1 编程模型差异

| 维度 | CkyClaw Framework | LangGraph |
|------|-------------------|-----------|
...
**核心差异**：CkyClaw 采用「Agent 为中心」的声明式模型（定义 Agent 的能力，Runner 驱动执行循环），LangGraph 采用「图为中心」的命令式模型（定义节点和边，状态机驱动状态流转）。

### 5.3.2 概念映射表

如果基于 LangGraph 重构，CkyClaw 的每个核心概念需要如何映射：

| CkyClaw 概念 | LangGraph 等价物 | 迁移难度 | 说明 |
|-------------|------------------|:--------:|------|
...
2. **企业级需求不匹配**：LangGraph 定位为通用编排框架，CkyClaw 的 RBAC/多租户/审计等企业能力与其无关
...
| 策略 | 说明 | 优先级 |
|------|------|:------:|
| **LangChain Tool 兼容层** | 允许 `@langchain.tool` 定义的工具在 CkyClaw 中使用 | P2 |
| **LangGraph Subgraph 桥接** | 允许将 LangGraph StateGraph 作为 CkyClaw 的 Tool 或 Workflow Step 调用 | P2 |
| **LangSmith 替代** | CkyClaw Tracing 已等价甚至超越 LangSmith 的核心功能（且本地化） | — |
