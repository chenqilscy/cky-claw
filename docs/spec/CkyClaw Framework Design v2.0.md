...
CkyClaw Framework 是一个 **Python Agent 运行时框架**，为 CkyClaw 平台提供底层 Agent 执行能力。

### 1.1 核心原则

| 原则 | 说明 |
|------|------|
| **框架无业务** | CkyClaw Framework 不包含用户/组织/权限等业务概念，只提供 Agent 运行原语 |
...
### 2.2 公共 API（`ckyclaw-framework/__init__.py`）

```python
"""CkyClaw Framework - Agent Runtime Framework"""
...
from ckyclaw_framework.handoff.filters import InputFilter

# === Tools ===
from ckyclaw_framework.tools.function_tool import function_tool
from ckyclaw_framework.tools.tool_group import ToolGroup
from ckyclaw_framework.tools.registry import ToolRegistry

# === MCP ===
from ckyclaw_framework.mcp.server import MCPServer
...
from ckyclaw_framework.session.backend import SessionBackend, SessionMetadata
...
# === Memory ===
from ckyclaw_framework.memory.memory import MemoryBackend, MemoryEntry, MemoryExtractor
...
# === Sandbox ===
from ckyclaw_framework.sandbox.sandbox import Sandbox
```
...
```python
from ckyclaw_framework import Agent, Runner, RunConfig, Session
from ckyclaw_framework.tracing.processor import TraceProcessor
from ckyclaw_framework.approval.handler import ApprovalHandler

# CkyClaw 实现 Trace Processor → MVP 默认导出到 PostgreSQL
...
# CkyClaw 实现自定义 Approval Handler → 对接审批工作流
class CkyClawApprovalHandler(ApprovalHandler):
...
config = RunConfig(
    ...
    approval_handler=CkyClawApprovalHandler(),
...
Runner 是 CkyClaw Framework 最核心的模块，负责驱动 Agent Loop。其内部分为以下子组件：
...
| 并发 Run | Runner 无状态，可多实例并行；并发度由应用层（CkyClaw）控制 |
...
class OTelTraceProcessor(TraceProcessor):
    """将 CkyClaw Framework Span 转换为 OTel Span 并通过 OTLP 导出。"""
...
| CkyClaw Span 字段 | OTel Attribute | 说明 |
|-------------------|---------------|------|
...
框架层 Session 聚焦于消息历史管理，不涉及业务实体（user_id / org_id 由 CkyClaw 应用层维护）：
...
清理策略由 CkyClaw 应用层的后台任务调用 `SessionBackend.delete()` 执行，框架本身不启动定时任务。
...
```python
# 内置 Guardrail 使用示例
from ckyclaw_framework.guardrails import (
...
class DockerSandbox(Sandbox):
    ...
    def __init__(
        self,
        image: str = "ckyclaw/sandbox:latest",
...
```bash
pip install ckyclaw-framework[litellm,postgres,mcp]
```
...
```python
class CkyClawFrameworkError(Exception):
    """CkyClaw Framework 基础异常"""

class AgentError(CkyClawFrameworkError):
    """Agent 相关错误"""

class RunnerError(CkyClawFrameworkError):
    """Runner 执行错误"""

class MaxTurnsExceeded(RunnerError):
    """超过最大执行轮次"""

class InputGuardrailError(CkyClawFrameworkError):
    """输入护栏拦截"""
    guardrail_name: str
    result: GuardrailResult

class OutputGuardrailError(CkyClawFrameworkError):
    """输出护栏拦截"""
    guardrail_name: str
    result: GuardrailResult

class ToolGuardrailError(CkyClawFrameworkError):
    """工具护栏拦截"""
    guardrail_name: str
    tool_name: str
    result: GuardrailResult

class ApprovalRejectedError(CkyClawFrameworkError):
    """审批被拒绝"""
    action_type: str

class ApprovalTimeoutError(CkyClawFrameworkError):
    """审批超时"""

class ModelProviderError(CkyClawFrameworkError):
    """模型调用失败"""
    model: str
    status_code: int | None

class MCPError(CkyClawFrameworkError):
    """MCP 协议错误"""
    server_name: str

class SandboxError(CkyClawFrameworkError):
    """沙箱执行错误"""
    exit_code: int
```
...
```python
# CkyClaw Framework 提供测试用 Mock
from ckyclaw_framework.testing import MockModelProvider, MockSession
...
| 维度 | OpenAI Agents SDK | CkyClaw Framework |
|------|-------------------|----------|
