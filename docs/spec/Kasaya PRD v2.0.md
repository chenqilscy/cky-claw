...
Kasaya 是基于自研 **Kasaya Framework** 构建的 AI Agent 管理与运行平台。Kasaya Framework汲取了 Claude Code、OpenAI Codex CLI、OpenAI Agents SDK、DeerFlow 等业界领先方案的优秀设计，提供一套开放、可扩展的 Agent 运行时基础设施。Kasaya 在此基础上构建企业级的 Agent 配置管理、多模式编排、执行可视化、人工监督、多渠道接入和 APM 监控等上层能力。**分层定位：**

| 层级 | 说明 |
|------|------|
| **Kasaya Framework（框架层）** | 自研 Agent 运行时基础设施。提供 Agent 定义、执行引擎（Runner）、编排（Handoff / Agent-as-Tool）、工具系统、MCP 集成、Session 管理、Guardrails、Tracing 等核心原语。未来可独立开源。 |
| **Kasaya 应用层** | 基于 Kasaya Framework 构建的企业级应用。提供 Web UI、多租户管理、RBAC 权限、人工监督、APM 仪表盘、IM 渠道接入等业务功能。 |
...
Agent 是 Kasaya Framework 中的核心执行单元。每个 Agent 是一个由 **Instructions + Model + Tools + Handoffs** 组成的声明式定义。**Agent 核心属性：**
...
Kasaya Framework 通过 Handoff 和 Agent-as-Tool 两个原语，支撑以下三大执行模式：
...
**Agent Team** 是 Kasaya 在单 Agent / 多 Agent 委派模型之上引入的 **团队级抽象**——将一组 Agent 绑定为可复用的协作单元，通过 **TeamProtocol（协作协议）** 定义成员间的交互方式，由 **Coordinator Agent** 按需调用。
...
Tool 是 Agent 与外部世界交互的桥梁。Kasaya Framework 支持以下工具类型：| 工具类型 | 说明 |
|----------|------|
...
当 Agent 配置了大量工具时（>20），将所有工具描述写入 Prompt 会消耗大量 Token。Kasaya Framework 支持 **ToolSearchTool** 模式——仅向 LLM 暴露一个 `search_tools(query)` 元工具，LLM 按需搜索并加载具体工具：| 特性 | 说明 |
|------|------|
...
```
yamlname: data-analysisversion: 1.2.0description: "数据分析领域知识——SQL 查询优化、图表生成建议、统计方法选择"author: Kasaya Teamtags: [data, sql, visualization]applicable_agents: ["data-analyst", "report-writer"]
...
> Session 详细设计（数据模型、后端实现规范、裁剪策略、TTL 管理）见《Kasaya Framework Design v2.0》第九章。
...
**Tripwire 触发机制：**1. Guardrail 函数执行检测逻辑（可以调用 LLM 进行语义分析）2. 返回 `GuardrailResult`，其中 `tripwire_triggered=true` 表示触发3. 触发后 Runner 立即中断当前执行，抛出对应异常（`InputGuardrailError` / `OutputGuardrailError` / `ToolGuardrailError`）4. Kasaya 应用层捕获异常，可转入审批工作流或直接向用户返回错误信息**执行模式：**
...
> Guardrails 详细设计（执行引擎、流水线、LLM-Based 模式、配置组合）见《Kasaya Framework Design v2.0》第十章。
...
| **结构化事实** | Agent 执行中积累的事实数据 | "项目使用 PostgreSQL 16（置信度 0.95）" |Kasaya 提供记忆管理 UI：| 能力 | 说明 |
|------|------|
...
> Memory 详细设计（提取流水线、向量检索、隐私生命周期管理）见《Kasaya Framework Design v2.0》第十一章。

### 2.13 Model Provider（模型提供商）

Kasaya Framework 采用 Provider-agnostic 设计，通过 LiteLLM 适配层支持多 LLM 提供商。**支持的主流厂商：**
...
| 私有部署 | vLLM / Ollama / LocalAI 等 | 自定义 Base URL |**Kasaya 模型厂商管理：**Kasaya 提供统一的模型厂商配置管理界面，管理员配置厂商连接信息后，Agent 配置时可从已启用的厂商和模型中选择。| 管理能力 | 说明 |
|---------|------|
...
> Runner Lifecycle Hooks 完整接口设计详见《Kasaya Framework Design v2.0》第六章 6.8 节。

#### 2.16.3 执行权限与安全

Kasaya 通过 **多层安全模型** 控制 Agent/Tool/Skill 的执行权限：| 层级 | 机制 | 控制点 |
|------|------|--------|
...
Kasaya 采用 **Kasaya Framework + Kasaya 应用** 的分层架构。Kasaya Framework作为 Python 库嵌入后端，提供 Agent 运行时能力；Kasaya 应用层在其上构建企业级管理与监控功能。
...
**：Kasaya Framework 无业务概念（无用户/组织/权限），Kasaya 应用层负责所有业务逻辑2. **声明式 Agent 配置**：Agent 通过 YAML 或代码声明式定义，支持版本管理3. **Provider-agnostic**：不绑定任何特定 LLM 提供商，通过 Model Provider 抽象适配多模型4. **Tracing-first 可观测性**：所有执行行为自动产生 Trace/Span，驱动可视化和 APM5. **MCP 标准化工具接入**：遵循 Model Context Protocol 标准，工具生态开放可扩展
...
| 分册 | 文件 | 章节 | 内容 |
|------|------|------|------|
| **Agent 编排** | 《Kasaya PRD-Agent 编排 v2.0》 | 四~六 | Agent 执行模式（Handoff/Agent-as-Tool/Team）、工具系统与 MCP、任务执行可视化 |
| **企业能力** | 《Kasaya PRD-企业能力 v2.0》 | 七~十 | IM 渠道接入、人工监督机制、APM 与可观测性、前端与用户界面 |
| **基础设施** | 《Kasaya PRD-基础设施 v2.0》 | 十一~十五 | API 设计、数据模型、用户系统与安全管理、部署与运维、非功能性需求 |

### 设计文档索引

| 文档 | 版本 | 职责 |
|------|------|------|
| 《Kasaya Framework Design v2.0》 | v2.0.0 | Agent 运行时库（Agent/Runner/Tool/Session/Tracing/Memory） |
| 《Kasaya API Design v1.2》 | v1.2.0 | 完整 REST API + WebSocket/SSE 事件协议 |
| 《Kasaya 应用层技术设计方案 v1.2》 | v1.2.0 | IM 渠道、前端 UI、APM 仪表盘、通知系统、配置/i18n 页面 |
| 《Kasaya 数据模型详细设计 v1.3》 | v1.3.0 | 全部实体列级 Schema（字段/约束/索引） |
...
| 术语 | 说明 |
|------|------|
| **Kasaya Framework** | Kasaya 自研的 Agent 运行时框架，提供 Agent 定义、执行引擎、编排、工具系统等核心原语 |
...
| **ProviderConfig** | Kasaya 中的模型厂商配置实体 |
| **ModelConfig** | Kasaya 中的具体模型配置实体 |
...
| 1.0.2 | 2026-04-01 | 更名为 Kasaya；精简技术实现细节，回归 PRD 定位 |
...
| **2.0.0** | **2026-04-01** | **架构重大变更：放弃 DeerFlow 依赖，引入自研 Kasaya Framework。新增 Handoff/Agent-as-Tool 编排模式、Guardrails 护栏、Tracing 链路追踪、Approval Mode 审批模式、Session 管理、Model Provider 多模型支持。所有 Agent/Session/Run 操作由 Kasaya 自主管理。** |
...
| 2.0.2 | 2026-04-02 | PRD 瘦身：技术细节抽取到《Kasaya 数据模型详细设计 v1.3》和《Kasaya 应用层技术设计方案 v1.2》。增强 Session（历史裁剪/并发安全）、Guardrails（6 个内置护栏）、Memory（置信度/向量检索）章节。更新 MVP 功能边界（Skills/Sandbox）。新增前端/IM/APM 章节到应用层设计文档的交叉引用。 |
...
|---------|---------------|----------------------|----------------|
| **Kasaya Framework** | Agent + Runner + 基础 Agent Loop | Memory（v2.2）、Skills（v2.2）、Sandbox（v2.6）、Checkpoint（v2.6）、Intent Detection（v2.6）、Cost Router（v2.6）、Evolution（M8） | — |
...
# | 假设 | 验证标准 |
|---|------|---------|
| 1 | Kasaya Framework Agent Loop 可稳定驱动多轮对话 | 10 轮对话无状态丢失 |
...
| 交付物 | 说明 |
|--------|------|
| 项目仓库初始化 | monorepo 结构（backend/ + frontend/ + kasaya-framework/） |
| Kasaya Framework 包骨架 | Agent、Runner、Tool、Session、Tracing 模块空壳 + 公共 API |
| Kasaya 后端骨架 | FastAPI 项目、DB 迁移脚本、基础中间件（认证、限流、错误处理） |
| Kasaya 前端骨架 | React + Vite + Ant Design + 路由框架 + 登录页 |
...
| 交付物 | 说明 |
|--------|------|
| Kasaya Framework Agent 定义 | Agent 数据类、声明式配置解析 |
| Kasaya Framework Runner | Agent Loop 实现（LLM 调用 → Tool 执行 → 消息追加 → 循环） |
| Kasaya Framework Function Tool | `@function_tool` 装饰器、ToolRegistry、自动 JSON Schema 生成 |
| Kasaya Framework Session | 基础会话管理（PostgreSQL 后端） |
...
| 交付物 | 说明 |
|--------|------|
| Kasaya Framework Handoff | Handoff 机制实现 + Runner 集成 |
| Kasaya Framework Agent-as-Tool | `Agent.as_tool()` 实现 |
| Kasaya Framework Tracing | Trace/Span 自动采集、TracingProcessor 接口 |
| Kasaya Trace Processor | 将 Trace 数据写入 PostgreSQL（MVP 默认）；可选开启 OTel 导出 |
...
| 交付物 | 说明 |
|--------|------|
| Kasaya Framework Approval Mode | suggest 模式实现 |
| Kasaya Framework Input Guardrail | 基础 Prompt 注入检测 |
| Kasaya 审批工作流 | 审批请求创建 → WebSocket 推送 → 批准/拒绝 |
