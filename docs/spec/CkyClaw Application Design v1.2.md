...
本文档描述 **Kasaya 应用层**的技术设计，覆盖 PRD 中框架层之上的企业级功能：

| 层级 | 文档 | 职责 |
|------|------|------|
| Kasaya Framework | 《Kasaya Framework Design v2.0》 | Agent 运行时库（Agent/Runner/Tool/Session/Tracing…） |
| **Kasaya 应用层** | **本文档** | Web UI、IM 渠道、APM 仪表盘、监督面板等业务功能 |
| PRD 总纲 | 《Kasaya PRD v2.0》 | 产品需求定义 |
...
| Consumer Group | `kasaya-workers`（支持多实例消费） |
...
IM 消息到达后，SessionRouter 按以下规则映射到 Kasaya Framework Session：
...
| 3b. 未绑定 | 回复引导消息："请输入 `/bind <token>` 绑定您的 Kasaya 账号" |
| 4. 绑定 | 用户在 Kasaya Web 端生成一次性 bind token → 在 IM 中输入 `/bind <token>` → 写入绑定表 |
...
| 查询类型 | PromQL 模板 |
|---------|-------------|
| 任务成功率 | `sum(rate(kasaya_run_total{status="completed"}[5m])) / sum(rate(kasaya_run_total[5m]))` |
| P95 延迟 | `histogram_quantile(0.95, sum(rate(kasaya_run_duration_bucket[5m])) by (le))` |
| Token 速率 | `sum(rate(kasaya_tokens_total[5m])) by (model)` |
| 成本趋势 | `sum(increase(kasaya_cost_usd_total[1h])) by (provider)` |
