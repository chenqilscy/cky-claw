...
| 状态 | 进行中 |
| 维护人 | Kasaya Team |
| 依赖 | Kasaya PRD v2.0（总纲）、Kasaya Framework Design v2.0 |

> 本文档是 Kasaya PRD v2.0 的分册，包含第四章（Agent 执行模式）、第五章（工具系统与 MCP）、第六章（任务执行可视化）。
...
Kasaya Framework 支持两种 Agent 编排模式：**Handoff（控制转移）

** 和 **Agent-as-Tool（嵌套调用）**。两种模式可独立使用或组合使用，由 LLM 或应用代码驱动编排决策。Kasaya 不干预编排逻辑，负责配置管理、执行可视化和人工监督。
...
> 代码驱动编排的完整示例（链式/条件/并行）见《Kasaya Framework Design v2.0》第四章和第六章。
...
### 4.8 Kasaya Agent 配置管理

Kasaya 提供声明式 Agent 管理界面：
...
Kasaya 支持 Agent 的非交互式执行——定时触发和批量输入两种模式。
...
> 数据模型（scheduled_runs / batch_runs 表）详见《Kasaya 数据模型详细设计 v1.3》，API 接口详见《Kasaya API Design v1.2》。**适用场景：**- "每天 9 点运行日报 Agent，生成各部门运营日报"（定时）。- "对 200 份简历逐一运行筛选 Agent"（批量）。- "每周一生成上周 Token 成本报告并邮件发送"（定时 + 通知）。
...
> TeamConfig 完整数据类定义、YAML 示例和 TeamRunner 执行策略详见《Kasaya Framework Design v2.0》4.5 节。

#### 4.11.2 内置 Team 模板

Kasaya 应用层预置以下 Team 模板，用户可基于模板快速创建团队或直接使用：
...
> Coordinator YAML 配置示例详见《Kasaya Framework Design v2.0》4.5 节。
...
Kasaya 支持 8 种协作协议，覆盖主流多 Agent 协作范式：
...
Kasaya 平台出厂即包含一组经过调优的 **内置 Agent 模板**、**Hosted Tool（内置工具组）** 和 **内置 Skill 包**，让用户开箱可用。
...
> 各工具组的函数签名和 JSON Schema 详见《Kasaya Framework Design v2.0》第八章。开发者可通过 `@function_tool` 注册自定义工具或通过 MCP Server 接入外部工具。
...
Function Calling 是 LLM 与外部工具交互的标准机制。Kasaya Framework 通过 Function Tool 装饰器注册工具函数，自动生成 JSON Schema 供 LLM 调用。**工具注册流程：** 开发者用 `@function_tool` 装饰器标注 Python 函数 → 框架自动从签名和 docstring 生成 JSON Schema → 工具注册到 ToolRegistry → Runner 将 Agent 可用工具的 Schema 附带给 LLM → LLM 返回 tool_calls 后 Runner 执行对应函数。

> 工具注册示例代码、ToolRegistry API 和 JSON Schema 生成细节详见《Kasaya Framework Design v2.0》第八章。
...
是标准化工具服务协议。Kasaya Framework 原生支持 MCP 集成：| 传输类型 | 说明 | 适用场景 |
|---------|------|---------|
...
```
1. Kasaya 后台配置 MCP Server 连接信息（URL/命令 + 认证方式）2. Kasaya Framework MCPServer 建立连接（stdio 启动子进程 / HTTP 握手）3. 调用 MCP list_tools 获取可用工具列表和 JSON Schema4. 工具注册到 ToolRegistry，归入 Agent 的 mcp_servers5. Agent 运行时，MCP 工具与 Function Tool 统一作为 tools 传递给 LLM6. LLM 调用 MCP 工具 → Kasaya Framework 通过 MCP 协议转发请求 → 返回结果
...
当 Agent 同时使用来自多个 MCP Server、多个 Skill、本地 Function Tool 的工具时，工具名称可能冲突。Kasaya Framework 通过命名空间自动隔离：**命名空间自动分配规则：**
...
当 Agent 配置的可用工具数量较多时，将所有工具的 JSON Schema 注入 Prompt 会消耗大量 Token 并降低 LLM 的工具选择精度。Kasaya Framework 实现 **ToolSearchTool** 模式来解决此问题：**工作流程：**
...
工具调用是 Agent 系统中最常见的异常来源。Kasaya Framework 采用"容错而非终止"策略：**默认行为：** 工具执行异常不会终止 Run，而是将错误信息作为工具结果返回给 LLM，由 LLM 决定重试、换用其他工具或通知用户。| 错误场景 | 处理方式 |
|---------|---------|
...
> 工具错误处理的完整配置项和示例代码详见《Kasaya Framework Design v2.0》第八章。

---

## 六、任务执行可视化

### 6.1 概述任务执行可视化是 Kasaya 的核心功能，基

于 Kasaya Framework Tracing 数据，以流程图形式实时展示 Agent 编排与执行过程。用户可以直观查看 Agent 间的 Handoff/调用关系、各节点的执行状态和日志，并对执行进行管理。
...
### 6.6 数据来源执行可视化的数据来源于 Kasaya Framework Tracing 系统：
...
Kasaya 后端注册自定义 Trace Processor，将 Tracing 事件实时推送到前端渲染执行图。
