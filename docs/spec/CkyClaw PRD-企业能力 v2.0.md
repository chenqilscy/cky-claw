...
| 状态 | 进行中 |
| 维护人 | Kasaya Team |
| 依赖 | Kasaya PRD v2.0（总纲）、Kasaya Framework Design v2.0、Kasaya 应用层技术设计方案 v1.2 |

> 本文档是 Kasaya PRD v2.0 的分册，包含第七章（IM 渠道接入）、第八章（人工监督机制）、第九章（APM 与可观测性）、第十章（前端与用户界面）。
...
Kasaya 支持多渠道接入，用户可通过 IM 应用、Web 端等多种方式与 Agent 交互。渠道层通过统一消息总线（MessageBus）连接 Kasaya Framework 的 Runner 执行引擎。
...
▼
│
│           Kasaya Framework Session       Kasaya Framework Runner                    │└─────────────────────────────────────────────────────────────────────┘
...
| 渠道 | 传输方式 | 流式输出 | 说明 |
|------|---------|---------|------|
| **Web 端** | REST + SSE | 是 | Kasaya 前端对话界面，支持流式输出和文件上传 |
...
| 渠道 | 状态 | 说明 |
|------|:----:|------|
| Slack | ✅ v2.5 | Socket Mode |
| 飞书/Lark | ✅ v2.5 | WebSocket |
| 企业微信 | ✅ v2.5 | 回调接口 |
| 钉钉 | ✅ v2.5 | 回调接口 |
| Discord | ✅ v2.5 | WebSocket |
| 自定义 Webhook | ✅ v2.5 | 通用出站 |
| API 直接接入 | ✅ | 第三方系统通过 REST + SSE 调用 |
...
| IM 端概念 | Kasaya Framework 映射 |
|----------|-------------|
| chat_id（群/私聊） | Session |
| user_id | Run Context metadata |
| InboundMessage | 用户输入 → Runner.run() |

> Channel Adapter 接口定义、MessageBus（Redis Streams）架构、Session 映射规则、渠道格式转换、分段推送策略、用户绑定流程等技术细节详见《Kasaya 应用层技术设计方案 v1.2》第一章。
...
Kasaya 提供全程人工监督能力，与 Kasaya Framework 的 Approval Mode 和 Guardrails 机制配合，确保 AI Agent 的行为安全可控。人类操作员可以随时观察任何 Agent 对话，实时干预或修正 Agent 行为。
...
### 8.3 与 Kasaya Framework 机制集成

| Kasaya 监督能力 | Kasaya Framework 底层机制 |
|-----------------|-----------------|
...
恢复监督员可随时暂停正在执行的 Agent。Kasaya Framework 将当前 RunState 序列化保存（含消息历史、待执行节点）。监督员修改或补充指令后恢复执行，Agent 从保存的状态继续。**RunState 序列化内容：**
...
### 8.6 审批工作流审批工作流基于 Kasaya Framework Approval Mode 扩展，可配置审批规则：
...
└── full-auto 模式：不触发（除非 Guardrail Tripwire）2. Kasaya Framework 调用 ApprovalHandler.request_approval()3. Kasaya ApprovalHandler 实现：     a. 创建 ApprovalRequest 记录（状态 = pending）     b. WebSocket 推送到监督面板审批队列     c. 同时发送通知（站内 + 邮件/Webhook）给匹配角色的审批人     d. Runner 执行挂起，等待审批结果4. 审批人操作：
...
Kasaya 的 APM 体系基于 Kasaya Framework 内建的 Tracing 系统。每次 Agent 执行自动产生 Trace，包含完整的 Span 层级结构：
...
Kasaya Framework 的 `TraceProcessor` 接口支持多种导出目标。Kasaya 应用层采用 **OTel 主路径 + 可选增强** 架构：| 导出目标 | 定位 | 用途 | 方式 |
|---------|------|------|------|
...
| **ClickHouse** | **可选（大规模）** | 海量 Trace 分析、高并发多维聚合（日均 > 100 万 Span 场景） | `ClickHouseTraceProcessor` 直写 |**推荐生产部署组合：** PostgresTraceProcessor（Token 审计）+ OTelTraceProcessor（Trace/Metrics）→ OTel Collector → Jaeger/Tempo + Prometheus + Grafana。**OpenTelemetry 集成策略（兼容而非依赖）：**Kasaya Framework 内部 Tracing 使用自研轻量 API（`Trace` / `Span` / `TraceProcessor`），**不依赖** OTel Python SDK。原因：| 因素 | 决策理由 |
|------|---------|
...
但通过 `OTelTraceProcessor` 提供**协议兼容**：- Kasaya Span → 转换为 OTel Span（W3C trace_id / span_id 格式）- 通过 OTLP gRPC/HTTP 推送到 OTel Collector- Collector 再分发到 Jaeger、Grafana Tempo 等后端存储- Collector 通过 Prometheus Remote Write 导出指标- Grafana 统一仪表盘查询 Jaeger/Tempo + Prometheus 数据源

```
Kasaya Framework Tracing (自研 API)        │
...
> 告警引擎评估周期与状态机、成本控制引擎（Pre-Run / Per-LLM 检查点）、模型降级执行器、指标查询层（PostgreSQL / Prometheus + Redis 缓存策略）等技术细节详见《Kasaya 应用层技术设计方案 v1.2》第三章。

#### 9.4.3 Token 开销优化设计

Kasaya 在系统架构的 **6 个层面** 内嵌 Token 节省机制：
...
> 各优化策略的实现细节（裁剪算法、缓存引擎、降级执行器）详见《Kasaya Framework Design v2.0》第六/九章和《Kasaya 应用层技术设计方案 v1.2》第三章。

### 9.5 Token 审计日志与统计

Token 审计是 Kasaya 对大模型使用的全链路追踪与分析体系。基于 Kasaya Framework Tracing 中的 LLM Span 数据，构建多维度的 Token 消耗审计、统计和预警能力。
...
```
> 项目结构、路由表、SSE 流式渲染机制、执行流程图（ReactFlow）映射规则、监督面板 WebSocket 事件协议等技术细节详见《Kasaya 应用层技术设计方案 v1.2》第二章。
