...
| **Framework 独立性不足** | `kasaya-framework` 与 `backend` 的边界模糊，Framework 尚不能独立作为 pip 包供外部开发者使用 | **高** |
...
| 任务 | 说明 |
|------|------|
| 发布为 pip 包 | `pip install kasaya-framework` 可独立使用 |
...
- 已有 10 个模板，扩展为可分享的市场
- 支持模板导入/导出、评分、评论
- 支持"一键部署"到用户的 Kasaya 实例

#### P2-3：Agents SDK 兼容层

- 允许 OpenAI Agents SDK 定义的 Agent 直接在 Kasaya 上运行
- 降低迁移成本，吸引 Agents SDK 用户

#### P2-4：LangChain Tool 桥接

- 允许 `@langchain.tool` 定义的工具在 Kasaya 中使用
...
当前架构                          目标架构
┌──────────────────┐      ┌──────────────────────────────┐
│ kasaya-framework│      │ kasaya-framework（独立 pip）  │
│  （editable 安装）│  →   │   ├── core（Agent/Runner/...）│
├──────────────────┤      │   ├── ext（MCP/Memory/Skill） │
│ backend          │      │   └── adapters（SDK 兼容层）  │
│  （依赖 framework）│      ├──────────────────────────────┤
├──────────────────┤      │ kasaya-server（Backend）     │
│ frontend         │      ├──────────────────────────────┤
└──────────────────┘      │ kasaya-ui（Frontend）        │
                          ├──────────────────────────────┤
                          │ kasaya-cli（命令行工具）NEW   │
                          └──────────────────────────────┘
```

### 4.2 新增 `kasaya-cli`（高价值、低投入）

一个类似 Claude Code / Codex CLI 的命令行工具：

```bash
# 在终端中直接与 Agent 对话
kasaya chat --agent code-reviewer

# 运行工作流
kasaya run --workflow data-pipeline --input data.csv

# 管理 Agent 配置
kasaya agent list
kasaya agent create --from-template code-reviewer
```
...
1. **Kasaya 的技术基础极其扎实** — 3220+ 测试、45 个迁移、37 个 API 模块、38 个页面，完成度在同类项目中罕见
2. **当前最大风险不是"能力不够"，而是"用户不够"** — 需要尽快拿到真实用户反馈
3. **Framework 独立化是最高优先级的架构决策** — 它决定了 Kasaya 是"一个项目"还是"一个生态"
