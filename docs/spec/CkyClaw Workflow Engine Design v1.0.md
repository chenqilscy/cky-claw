...
CkyClaw 当前仅有两种 Agent 编排模式：**Handoff**（LLM 驱动的 Agent 间交接）和 **Agent-as-Tool**（LLM 决定何时调用子 Agent）。两者都依赖 LLM 自主决策，无法实现确定性的程序化编排（顺序链、并行扇出/扇入、条件分支、循环迭代）。

本方案在 CkyClaw Framework 中新增 **Workflow Engine**，提供声明式 DAG 编排能力，后端提供持久化和 REST API，前端提供可视化拖拽编辑器。
...
### Framework 层 — `ckyclaw-framework/ckyclaw_framework/workflow/`
...
文件：`ckyclaw-framework/ckyclaw_framework/workflow/engine.py`
...
**新建文件**（`ckyclaw-framework/ckyclaw_framework/workflow/`）：
...
**修改文件**：
- `ckyclaw_framework/__init__.py` — 新增 `# === Workflow ===` 导出段
- `ckyclaw_framework/tracing/span.py` — SpanType 新增 `WORKFLOW_STEP = "workflow_step"`

**测试**：每个步骤类型的执行、条件求值、DAG 验证、嵌套规则、output_key 冲突、重试逻辑、取消流程、超时。

完成后开发者可以纯 Python 定义并运行工作流：
```python
from ckyclaw_framework import Workflow, AgentStep, ParallelStep, WorkflowEngine
...
1. **Phase 1 验证**：`cd ckyclaw-framework && uv run pytest tests/ -q` — 单元测试覆盖每种步骤类型、条件求值、DAG 验证、嵌套规则、重试逻辑、取消、超时
