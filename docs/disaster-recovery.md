...
Kasaya 灾备方案围绕 **RTO < 4 小时、RPO < 1 小时** 目标设计，覆盖 PostgreSQL 和 Redis 两大数据组件。
...
```bash
# 设置连接信息
export PGHOST=localhost PGPORT=5432 PGUSER=kasaya PGPASSWORD=your_password PGDATABASE=kasaya
export REDIS_HOST=localhost REDIS_PORT=6379

# 执行备份
./scripts/backup.sh /var/backups/kasaya
```
...
# 查看备份日志
docker logs kasaya-backup

# 手动触发一次备份
docker exec kasaya-backup bash /scripts/backup.sh
```

### 验证备份

```bash
./scripts/backup-verify.sh /var/backups/kasaya
```
...
▸ PostgreSQL 备份
  ✓ 最新备份: kasaya_20260405_020000.dump (45M)
  ✓ 文件完整性验证通过（35 个对象）
  ✓ 备份文件数量: 28
...
```bash
# 恢复 PostgreSQL + Redis
./scripts/restore.sh /var/backups/kasaya/postgresql/kasaya_20260405_020000.dump \
                     /var/backups/kasaya/redis/dump_20260405_020000.rdb

# 仅恢复 PostgreSQL
./scripts/restore.sh /var/backups/kasaya/postgresql/kasaya_20260405_020000.dump

# 跳过确认提示（自动化场景）
RESTORE_CONFIRM=yes ./scripts/restore.sh /var/backups/kasaya/postgresql/kasaya_20260405_020000.dump
```
