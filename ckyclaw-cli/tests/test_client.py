...
class TestCkyClawClient:
    """API 客户端测试。"""

    def test_default_url(self) -> None:
        """默认 URL。"""
        client = CkyClawClient()
        assert client.base_url == "http://localhost:8000"

    def test_env_url(self) -> None:
        """从环境变量读取 URL。"""
        with patch.dict("os.environ", {"CKYCLAW_URL": "http://example.com:9000"}):
            client = CkyClawClient()
            assert client.base_url == "http://example.com:9000"

    def test_explicit_url(self) -> None:
        """显式传入 URL。"""
        client = CkyClawClient(base_url="http://test:1234/")
    ...
    def test_login(self, mock_server: tuple) -> None:
        ...
        client = CkyClawClient(base_url=url)
    ...
    def test_list_agents(self, mock_server: tuple) -> None:
        ...
        client = CkyClawClient(base_url=url, token="jwt")
    ...
    def test_connection_error(self) -> None:
        """连接失败。"""
        client = CkyClawClient(base_url="http://127.0.0.1:1")
