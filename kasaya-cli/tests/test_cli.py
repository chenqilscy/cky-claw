...
class TestVersion:
    ...
    def test_version_output(self) -> None:
        ...
        assert "kasaya-cli v0.1.0" in result.output
...
class TestChatCommand:
    ...
    @patch("kasaya_cli.chat._get_input", side_effect=["exit"])
    @patch("kasaya_cli.chat.console")
    def test_exit_immediately(self, mock_console: MagicMock, mock_input: MagicMock) -> None:
        """输入 exit 立即退出。"""
        result = runner.invoke(app, ["chat"])
        assert result.exit_code == 0

    @patch("kasaya_cli.chat._get_input", side_effect=["quit"])
    @patch("kasaya_cli.chat.console")
    ...
    @patch("kasaya_cli.chat._get_input", side_effect=["clear", "exit"])
    @patch("kasaya_cli.chat.console")
    ...
    @patch("kasaya_cli.chat._get_input", side_effect=KeyboardInterrupt)
    @patch("kasaya_cli.chat.console")
    ...
    @patch("kasaya_cli.chat.Runner")
    @patch("kasaya_cli.chat._get_input", side_effect=["你好", "exit"])
    @patch("kasaya_cli.chat.console")
    ...
    @patch("kasaya_cli.chat.Runner")
    @patch("kasaya_cli.chat._get_input", side_effect=["测试", "exit"])
    @patch("kasaya_cli.chat.console")
