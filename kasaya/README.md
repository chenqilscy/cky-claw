...
**Kasaya** 是一个声明式 Python Agent 运行时框架。它提供 Agent 定义、LLM 多模型适配、工具编排、Guardrails 护栏、Handoff 交接、Tracing 链路追踪等核心能力，让你用最少的代码构建可靠的 AI Agent。
...
```bash
pip install kasaya[postgres]   # PostgreSQL Session 后端
pip install kasaya[redis]      # Redis 缓存
pip install kasaya[mcp]        # MCP 协议支持
```
...
```python
from kasaya import Agent, Runner, function_tool
...
```python
from kasaya import Agent, Runner, RegexGuardrail
...
```python
from kasaya import Agent, Handoff, Runner
