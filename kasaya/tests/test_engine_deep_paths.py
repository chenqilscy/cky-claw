...
from kasaya.workflow.engine import (
    WorkflowEngine,
    _eval_conditional_inline,
    _run_loop_step,
    _run_sub_step,
)
from kasaya.workflow.result import StepStatus, WorkflowResult, WorkflowStatus
...
class TestLoopStepCancel:
    ...
    async def test_loop_cancel_in_body(self) -> None:
        ...
        with patch("kasaya.workflow.engine._run_agent_step", side_effect=_mock_run_agent), \
...
class TestWorkflowCancelMidExecution:
    ...
    async def test_cancel_during_execution(self) -> None:
        ...
        with patch("kasaya.workflow.engine.Runner") as MockRunner:
...
class TestLoopIterationOutput:
    ...
    async def test_loop_with_counter(self) -> None:
        ...
        with patch("kasaya.workflow.engine._run_agent_step", side_effect=_fake_agent_step):
            result = await WorkflowEngine.run(
...
class TestWorkflowDeadlineTimeout:
    ...
    async def test_deadline_triggers_timeout(self) -> None:
        ...
        with patch("kasaya.workflow.engine._run_agent_step", side_effect=_slow_agent_step):
            result = await WorkflowEngine.run(
...
class TestDAGContinueOnSkip:
    ...
    async def test_skip_causes_non_empty_ready(self) -> None:
        ...
        with patch("kasaya.workflow.engine._run_agent_step", side_effect=_tracking_agent_step):
            result = await WorkflowEngine.run(
...
class TestLoopCancelInMultiStepBody:
    ...
    async def test_cancel_between_body_steps(self) -> None:
        ...
        with patch("kasaya.workflow.engine._run_agent_step", side_effect=_agent_step_with_cancel), \
             pytest.raises(asyncio.CancelledError):
                await _run_loop_step(loop, {}, result, resolver, WorkflowRunConfig(), cancel, None, [])
