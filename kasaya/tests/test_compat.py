...
class TestWrapSdkToolFn:
    ...
    async def test_sdk_style_two_args(self) -> None:
        """SDK 风格 (ctx, args_json) → Kasaya **kwargs。"""
    ...
    def test_kasaya_style_passthrough(self) -> None:
        """Kasaya 原生风格（非两参数）直接返回。"""
...
class TestFromOpenaiHandoff:
    ...
    def test_dict_with_kasaya_agent(self) -> None:
        """目标已经是 Kasaya Agent。"""
...
class TestFromOpenaiAgent:
    ...
    def test_kasaya_agent_passthrough(self) -> None:
        """Kasaya Agent 直接透传。"""
...
class TestConvertModelSettings:
    ...
    def test_kasaya_instance_passthrough(self) -> None:
        """Kasaya ModelSettings 直接透传。"""
...
class TestSdkAgentAdapter:
    ...
    def test_convert_already_kasaya(self) -> None:
        """Kasaya Agent 传入 convert_agent 直接透传。"""
        adapter = SdkAgentAdapter()
        original = Agent(name="native")
...
class TestEdgeCases:
    ...
    def test_from_openai_tool_kasaya_function_tool(self) -> None:
        """传入 Kasaya FunctionTool 对象也能正常转换。"""
