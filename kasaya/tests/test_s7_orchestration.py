...
from kasaya.orchestration import (
    ExecutionPlan,
    GuardCheckResult,
    PlanGuard,
    PlanGuardResult,
...
class TestInMemoryMailboxBackend:
    ...
    async def test_send_and_receive(self) -> None:
        """发送并接收消息。"""
        from kasaya.mailbox import InMemoryMailboxBackend, MailboxMessage
    ...
    async def test_mark_read_nonexistent(self) -> None:
        """标记不存在的消息已读（静默跳过）。"""
        from kasaya.mailbox import InMemoryMailboxBackend
