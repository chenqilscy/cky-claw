...
from kasaya.approval.mode import ApprovalDecision, ApprovalMode, ApprovalRejectedError, classify_tool_risk
from kasaya.artifacts.store import ArtifactStore, _estimate_token_count
from kasaya.checkpoint import Checkpoint
from kasaya.debug.controller import DebugStoppedError
...
from kasaya.runner.hooks import _invoke_hook
...
async def _build_system_message(
    ...
    if config and config.template_variables and "{{" in text:
        from kasaya.agent.template import render_template
    ...
    if agent.response_style:
        from kasaya.agent.response_style import get_response_style_prompt
...
async def _build_system_messages(
    ...
    if config.memory_backend is not None and config.memory_user_id:
        from kasaya.memory.injector import MemoryInjector
...
async def _execute_tool_calls(
    ...
    *,
    ...
    if _tool_middleware:
        from kasaya.tools.middleware import ToolMiddlewarePipeline
    ...
    async def _run_one(tc: ToolCall) -> None:
        ...
        try:
            ...
            if _middleware_pipeline is not None:
                from kasaya.tools.middleware import ToolExecutionContext as _TEC  # noqa: N814
                _mw_ctx = _TEC(
                    tool_name=tc.name,
                    arguments=arguments,
