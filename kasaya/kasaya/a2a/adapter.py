...
A2AAdapter 将 Kasaya 内部的 Agent + Runner 执行模型，
...
from kasaya.a2a.task import A2ATask, TaskArtifact, TaskStatus
...
class A2AAdapter:
    ...
    def agent_to_card(
        ...
        *,
        ...
        """从 Kasaya Agent 实例生成 A2A Agent Card。

        Args:
            agent: Kasaya Agent 实例（duck typing，需有 name/description/tools 属性）。
            url: A2A 端点 URL。
    ...
    def task_to_messages(self, task: A2ATask) -> list[dict[str, Any]]:
        """将 A2A Task 的输入消息转换为 Kasaya Runner 格式。
