...
# 工作目录限制（防目录穿越）
_FILE_OPS_BASE_DIR = os.environ.get("KASAYA_FILE_OPS_BASE_DIR", "/tmp/kasaya-files")
...
async def database_query(
    ...
    Args:
        ...
        connection_string: 数据库连接字符串（如未提供，使用环境变量 KASAYA_DB_QUERY_DSN）
    ...
    dsn = connection_string or os.environ.get("KASAYA_DB_QUERY_DSN", "")
    if not dsn:
        return "错误：未配置数据库连接。请设置 KASAYA_DB_QUERY_DSN 环境变量或传入 connection_string 参数。"
