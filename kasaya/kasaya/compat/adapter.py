"""OpenAI Agents SDK → Kasaya Framework 适配器。

将 OpenAI Agents SDK 风格的 Agent / Tool / Handoff / Guardrail 定义
转换为 Kasaya Framework 原生对象。支持两种输入格式：
...
============================================  =============================================
OpenAI Agents SDK                             Kasaya Framework
============================================  =============================================
...
def from_openai_tool(sdk_tool: Any) -> FunctionTool:
    """将 OpenAI SDK FunctionTool 或等价 dict 转换为 Kasaya FunctionTool。
...
def _convert_tool_from_dict(d: dict[str, Any]) -> FunctionTool:
    ...
    # 函数：优先 on_invoke_tool（SDK 风格），其次 fn（Kasaya 原生）
    fn = d.get("on_invoke_tool") or d.get("fn")

    # 如果 fn 接收 (ctx, args_json_str)，包装为 Kasaya 风格 (ctx, **kwargs)
...
def _wrap_sdk_tool_fn(fn: Callable[..., Any]) -> Callable[..., Any]:
    """包装 SDK 风格工具函数为 Kasaya 兼容签名。

    SDK 工具函数签名: ``async (ctx, args_json: str) -> str``
    Kasaya 工具函数签名: ``async (**kwargs) -> str``

    如果函数已经是 Kasaya 风格（接收 **kwargs），直接返回。
    """
...
def from_openai_handoff(sdk_handoff: Any, *, agent_cache: dict[str, Agent] | None = None) -> Handoff:
    """将 OpenAI SDK Handoff 或等价 dict 转换为 Kasaya Handoff。
...
def from_openai_guardrail(
    ...
    *,
    ...
    """将 OpenAI SDK Guardrail 或等价 dict 转换为 Kasaya Guardrail。
    ...
    Returns:
        Kasaya ``InputGuardrail`` 或 ``OutputGuardrail``。
    """
...
def _wrap_guardrail_fn(fn: Callable[..., Any]) -> Callable[..., Awaitable[GuardrailResult]]:
    ...
    Kasaya 签名: ``async (run_context, text) -> GuardrailResult``
    """
...
def from_openai_agent(sdk_agent: Any, *, agent_cache: dict[str, Agent] | None = None) -> Agent:
    """将 OpenAI SDK Agent 或等价 dict 转换为 Kasaya Agent。
...
def _convert_model_settings(sdk_settings: Any) -> ModelSettings | None:
    """转换 SDK ModelSettings 为 Kasaya ModelSettings。"""
...
class SdkAgentAdapter:
    """OpenAI Agents SDK → Kasaya Framework 的高级适配器。
    ...
    async def run(
        ...
        *,
        ...
        """使用 Kasaya Runner 运行 Agent。

        如果 agent 不是 Kasaya Agent 实例，会先转换。

        Args:
            agent: Kasaya Agent 或 SDK Agent。
            input_text: 用户输入文本。
            max_turns: 最大执行轮次。
